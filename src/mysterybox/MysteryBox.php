...
class MysteryBox{
	...
	public function __construct(array $data){
		...
		if(count($data["rgb"]) !== 3){
			throw new \Exception("RGB phải chứa 3 số");
		}
		$this->rgb = array_values($data["rgb"]);
		
		if(empty($data["items"])){
			throw new \Exception("Hộp bí ẩn phải có ít nhất một vật phẩm làm phần thưởng");
		}
	...
	public function grantItem(Player $player) : array{
		...
		$display = "Không thể chọn item - chance không khớp";
		...
		while($tries < 100 and $item->isNull()){
			...
			if(mt_rand(1, 99) <= $d["chance"]){
				...
				if($item->isNull() == false){
					...
					$display = "Không thể cấp item, định dạng item không hợp lệ";
				}
			}
		}
