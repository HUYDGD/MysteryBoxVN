...
class EventListener implements Listener{
	...
	public function onInteract(PlayerInteractEvent $event) : void{
		...
		if($tile instanceof MysteryTile){
			...
			if($tile->in_use){
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &r&7Không thể mở Mystery Box vì đang có người sử dụng!"));
				return;
			}
			...
			if($box->canOpen($player)){
				...
				$player->sendTip(TF::colorize($this->getCore()->getConfig()->get("prefix")." &r&7Bạn không có chìa khóa nào để mở &e{$box->getName()}"));
			}
		}
	}
